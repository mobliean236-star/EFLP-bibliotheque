/* --------------- CONFIG: PROFESSEURS --------------- */
/* Modifie les comptes ici si tu veux */
const comptes = {
...
function loadData() {
  return JSON.parse(localStorage.getItem(STORAGE_KEY) || '{"books":[],"users":[]}');
}
...
if (!localStorage.getItem(STORAGE_KEY)) {
  saveData({ books: [], users: Object.keys(comptes) });
}
...
/* Rendu liste avec recherche & actions */
...
function renderBooks() {
  ...
  filtered.forEach((b) => {
    ...
    if (b.status === "available") borrowBtn.textContent = "Emprunter";
    else borrowBtn.textContent = "Retourner";
...
/* --------------- ACTIONS: emprunter / retourner / details / delete --------------- */

function promptBorrow(bookId) {
  const data = loadData();
  const book = data.books.find(b => b.id === bookId);
  if (!book) return alert("Livre introuvable.");
  // demander classe emprunteuse et nom élève
...
btnQR?.addEventListener("click", () => {
  ...
  qrcodeBox.innerHTML = "";
  if (!url) { alert("Entre une URL pour générer le QR (ex: ton site GitHub Pages)"); return; }
