document.addEventListener("DOMContentLoaded", () => {
    ...
    filtered.forEach(b=>{
      ...
      li.innerHTML=`
        <div class="book-left">
          ...
          ${emprunt?`<span class="badge">Emprunté: ${emprunt.eleve}</span>`:`<span class="badge">Disponible</span>`}
        </div>
        <div class="actions">
  ...
  window.borrowBook=function(bookId){
    ...
    if(!eleve) return;
    loans.unshift({id:crypto.randomUUID(), bookId, eleve, date:new Date().toISOString()});
    save(); afficherLivres(); afficherEmprunts();
  }
  ...
  function afficherEmprunts(){
    ...
    loans.forEach(l=>{
      ...
      div.innerHTML=`<strong>${book?.titre||"Livre supprimé"}</strong> - Élève: ${l.eleve} - ${new Date(l.date).toLocaleDateString()}`;
  ...
  btnExport.addEventListener("click",()=>{
    const blob=new Blob([JSON.stringify({books,loans},null,2)],{type:"application/json"});
    const url=URL.createObjectURL(blob);
    const a=document.createElement("a"); a.href=url; a.download="bibliotheque.json"; a.click(); URL.revokeObjectURL(url);
  });
  ...
  fileImport.addEventListener("change",e=>{
    ...
    reader.onload=ev=>{
      try{
        ...
        books=parsed.books; loans=parsed.loans; save();
        afficherLivres(); afficherEmprunts(); alert("Import réussi !");
